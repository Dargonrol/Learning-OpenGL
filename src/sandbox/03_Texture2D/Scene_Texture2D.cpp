...
namespace Scene
{
    ...
    int Scene_Texture2D::Init()
    {
        ...
        proj = glm::ortho(0.0f, (float)sm_->GetRenderer().app.GetWindowWidth(), 0.0f, (float)sm_->GetRenderer().app.GetWindowHeight(), -1.0f, 1.0f);
    ...
    {
        sm_->GetRenderer().app.SetTitle("Texture 2D test"sv);
        shader->Bind();
        texture->Bind(0);
    }
