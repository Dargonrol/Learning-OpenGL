...
namespace Scene
{
    ...
    int Scene_Simple3D_Camera::Init()
    {
        ...
        m_camera->SetAspectRatio(static_cast<float>(sm_->GetRenderer().app.GetWindowWidth()) / static_cast<float>(sm_->GetRenderer().app.GetWindowHeight()));
    ...
    void Scene_Simple3D_Camera::OnLeave()
    {
        ...
        sm_->GetRenderer().app.ReleaseMouse();
    }
    ...
    void Scene_Simple3D_Camera::HandleInput(const float deltaTime)
    {
        ...
        {
            sm_->GetRenderer().app.CaptureMouse();
            m_camera->enableMouseControl = true;
        }
